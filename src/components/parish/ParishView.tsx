...
export function ParishView() {
  ...
  return (
    <div className="space-y-4 pb-24 md:pb-12 max-w-6xl xl:max-w-7xl mx-auto px-3 sm:px-6 pt-3 sm:pt-4">
      {/* Sub-tab Navigation */}
      <div className="bg-white dark:bg-slate-900 border border-orthodox-gold/30 rounded-2xl p-1.5 shadow-sm flex space-x-1 max-w-xl mx-auto sm:mx-0">
        {subTabs.map((t) => (
          <button
            key={t.id}
      ...
      {subTab === 'schedule' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="font-serif font-bold text-base sm:text-xl text-orthodox-navy dark:text-orthodox-gold-light">
              {locale === 'ja'
                ? '大阪ハリストス正教会 奉事日程'
                : locale === 'ru'
            ...
            <button
              onClick={handlePrint}
              className="flex items-center space-x-1 text-xs py-1.5 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 font-semibold text-slate-700 dark:text-slate-200 shadow-sm"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {PARISH_SCHEDULE_2026.map((s) => (
              <div
                key={s.id}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:border-orthodox-gold/60 transition-all flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <h4 className="font-bold text-base text-slate-900 dark:text-white mt-0.5 line-clamp-2">
                          {s.title[locale]}
                        </h4>
                      </div>
                    </div>
                  ...
                  {s.notes && (
                    <div className="mt-2.5 text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/40 p-2 rounded-lg">
                      {s.notes[locale]}
                    </div>
                  )}
                </div>
                ...
                {s.dutyGroup && (
                  <div className="mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-1.5 text-slate-600 dark:text-slate-400">
                      ...
                      <span>{locale === 'ja' ? '当番: ' : locale === 'ru' ? 'Дежурные: ' : 'Duty: '}</span>
                      <span className="font-bold text-orthodox-navy dark:text-orthodox-gold-light">
                        &lt;{s.dutyGroup}&gt;
                      </span>
                      {s.dutyPeople && s.dutyPeople.length > 0 && (
                        <span className="truncate max-w-[120px] sm:max-w-none"> ({s.dutyPeople.join(', ')})</span>
                      )}
                    </div>
                  </div>
                )}
      ...
      {subTab === 'bulletin' && (
        <div className="space-y-4">
          <h3 className="font-serif font-bold text-base sm:text-xl text-orthodox-navy dark:text-orthodox-gold-light px-1">
